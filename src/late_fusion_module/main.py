...
def main():
    ...
    CONFIG = {
        ...
        'alpha_fixed': None          # used if learnable_alpha=False and no confidence (0.5)
    }

    print("Late Fusion Training (confidence-conditioned α)")
    print("Configuration:")
    ...
    # datasets & loaders (keep shuffle=False to preserve order for analysis vs. confidences)
    ...
    model = LateFusionModel(
        audio_dim=CONFIG['input_dim'],
        text_dim=CONFIG['input_dim'],
        out_dim=CONFIG['hidden_dim'],
        num_classes=num_classes,
        dropout=CONFIG['dropout'],
        alpha=(CONFIG['alpha_fixed'] if not CONFIG['learnable_alpha'] else None),
    ...
    trainer = LateFusionTrainer(
        ...
        use_conf_as_alpha=CONFIG['use_conf_as_alpha'],
        max_grad_norm=1.0
    )
    ...
    trainer.save_plots(OUTPUT_DIR)

    # ---- test ----
    print("Final Test Evaluation")
    test_results = trainer.evaluate(test_loader, return_predictions=True)
    # test_results includes: predictions, labels (if return_predictions), and alpha_used

    # Quartiles by ASR confidence (use test_confidences directly; loaders were not shuffled)
    conf_rows = bin_stats(
        values=test_confidences,
    ...
    # Quartiles by text reliance: in late fusion, g_text = 1 - alpha_used
    alpha_used = test_results['alpha_used']
    gate_text = 1.0 - alpha_used
    gate_rows = bin_stats(
        values=gate_text,
        preds=test_results['predictions'],
        labels=test_results['labels'],
    ...
    print("\nF1 by text-reliance quartiles (1 - alpha_used):")
    for r in gate_rows:
        print(f"[{r['bin_lo']:.3f}, {r['bin_hi']:.3f}]  n={r['n']:4d}  "
              f"Acc={r['acc']:.3f}  MacroF1={r['f1_macro']:.3f}")

...
class LateFusionTrainer:
    def __init__(self, model, device=None,
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, mode='max', factor=0.5, patience=lr_plateau_patience, min_lr=1e-6, threshold=1e-4, cooldown=0
        )
        print(f"LR: {self.optimizer.param_groups[0]['lr']:.2e}")

        # Late fusion mixes probabilities → train with NLLLoss on log-probs
    ...
    def _forward_model(self, batch, train: bool):
        ...
        # Pass confidence only if we want to use it as alpha
        kwargs = {}
    ...
    def train(self, train_loader, val_loader, num_epochs, patience=10):
        ...
        for epoch in range(num_epochs):
            ...
            # Compute α stats for this epoch
            alpha_mean = float(val_res['alpha_used'].mean()) if val_res['alpha_used'].size else 0.0
            alpha_std  = float(val_res['alpha_used'].std()) if val_res['alpha_used'].size else 0.0
    ...
    def save_plots(self, output_dir):
        """
        Call after training to save:
        - loss_curve.png (train vs val loss)
        - alpha_curve.png (alpha mean ± std)
        """
        # Import here to avoid circulars if your plot fn lives in utils

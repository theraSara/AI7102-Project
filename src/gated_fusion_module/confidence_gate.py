...
class ConfidenceGateMLP(nn.Module):
    def __init__(self, hidden_dim=256, gate_hidden=128, input_extra_dims=0):
        ...
        self.log_tau = nn.Parameter(torch.tensor(0.0))  # softmax temperature

    def forward(self, x):
        # x = self.norm(x)
        logits = self.gate_network(x)
        tau = 0.5 + 1.5 * torch.sigmoid(self.log_tau)     # τ ∈ [0.5, 2.0], no clamp/exp
        gates = F.softmax(logits / tau, dim=-1)
        return gates, logits
